...
export default function ContactForm() {
  ...
  // 日程
  const [arrivalDate, setArrivalDate] = useState(""); // 到着希望日（必須）
  const [useDate, setUseDate] = useState("");         // ご利用日（必須）
  const [shipDate, setShipDate] = useState("");       // 発送予定日（任意）

  // 会場
const [pref, setPref] = useState("");
const [venueName, setVenueName] = useState(""); // 会場名
const [venueAddress, setVenueAddress] = useState(""); // 住所
  ...
  const [sent, setSent] = useState<null | { id: string }>(null);
  const [error, setError] = useState<string | null>(null);

  // 割引関連
    const [promo, setPromo] = useState<"" | "photo5000">("");
    const [photoConsent, setPhotoConsent] = useState(false); // 掲載許諾（ぼかし対応OK）
    const [review, setReview] = useState(""); // ご感想（任意だが推奨）
  ...
  const promoParam = (sp.get("promo") ?? "").toLowerCase();
useEffect(() => {
    if (promoParam === "photo5000") setPromo("photo5000");
    }, [promoParam]);

  // フロント側バリデーション（最小日・相関チェック）
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // if (shipDate && !isBefore(shipDate, arrivalDate)) {
    //   return setError("発送予定日は到着希望日の前日までをご選択ください。");
    // }
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <label className="grid gap-1 text-sm">
            ...
            <input
                type="date"
                value={shipDate}
                min={useDate || minDate} // ご利用日が決まっていればその日以降を選択可
      ...
      <label className="inline-flex items-center gap-2 text-sm">
        ...
        />
        <Link href="/privacy" className="underline">プライバシーポリシー</Link>及び<Link href="/disclaimer" className="underline">免責事項</Link>に同意します
      </label>
